...
- The check is an internal method `(d *Detector) isProhibitedLang(msg string)` (`lib/tgspam/detector.go:1180`) called inline in `Detector.Check` right after the short-msg-flood block and before the stop-words block (`detector.go:274-282`), gated by `len(d.ProhibitedScripts) > 0` so disabled mode does no work
...
- `ResolveProhibitedScripts(names []string) (map[string]*unicode.RangeTable, error)` lives in `lib/tgspam/prohibited.go` (exported for cross-package use) and is the single resolver used by both `validateSettings` (`app/main.go:419`, fail fast on an unknown name) and `makeDetector` (`app/main.go:791`, error already caught by validation so it discards it). It expands friendly aliases via `scriptAliases` (chinese→Han, russian/ukrainian→Cyrillic, japanese→Hiragana+Katakana, arabic, korean→Hangul, hebrew, thai, greek), matches other names case-insensitively against `unicode.Scripts`, skips empty/whitespace entries, and errors naming an unknown entry
