...
- The check is an internal method `(d *Detector) isProhibitedLang(msg string)` (`lib/tgspam/detector.go:1180`) called inline in `Detector.Check` right after the short-msg-flood block and before the stop-words block (`detector.go:274-282`), gated by `len(d.ProhibitedScripts) > 0 && d.ProhibitedLangsMin > 0` so both an empty script list and a zero min (treated as disabled) do no work
