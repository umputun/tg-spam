...
- Because the flattened text is non-empty, the message survives the `listener.go:390-391` empty-text guard automatically; forwarded articles (`forward_origin` set) also work. A rich message with no extractable text (media-only) still yields empty `Text` and is dropped by that guard — accepted, since there is no content to check
...
- Channel `SenderChat` flow works transparently: `bot.OnMessage` already maps `checkUserID = msg.SenderChat.ID` and the locator stores under the same ID; anonymous admin and linked-channel posts skip `OnMessage` entirely (listener.go:409-413)
...
- Media-only messages (photo/video/document with no caption, GIFs included) therefore count toward `total`: they reach `AddMessage` with empty text (`listener.go:390-391` passes them on `msg.Image`/`WithVideo`/`WithVideoNote`/`WithDocument`) and `isShortMsgFlood` treats length 0 as short. This is intentional — image-flood probing is a real spam pattern — but it means an unapproved user posting N caption-less photos accumulates N, not 1
...
### Document Detection
- `--meta.documents-only` / `META_DOCUMENTS_ONLY` (Settings.Meta.DocumentsOnly) flags a message carrying a document (file attachment) whose text is empty or shorter than `--min-msg-len`. Disabled by default. Mirrors `VideosCheck`/`AudioCheck` exactly: `DocumentsCheck(minTextLen int)` in `lib/tgspam/metachecks.go`, gated in `makeDetector` by the setting, wired through `spamcheck.MetaData.HasDocument` (`bot.OnMessage`) and `bot.Message.WithDocument` (`transform`, `app/events/events.go`). Also exposed to Lua as `req.meta.has_document`
...
- The intake guard at `listener.go:390-391` gained `!msg.WithDocument` unconditionally — not gated on the setting, same treatment as `WithVideo`/`WithVideoNote`. With the check *disabled*, caption-less documents now reach the detector and locator where they previously did not. In `Detector.Check`, four identity-based checks run before the min-length gate (`detector.go:338`) and can each flag spam on their own: `isStopWord` (`detector.go:300`) matches against username and user ID as well as message text, so it can fire on an empty message; the meta-checks loop (`detector.go:309`) makes `UsernameSymbolsCheck` reachable, since it is message-independent; the Lua checks loop (`detector.go:314`) now sees this message class; and CAS (`detector.go:324`) — enabled by default (`app/main.go:81`, `default:"https://api.cas.chat"`) — runs one lookup per such message and can permanently ban a CAS-listed user. Content checks find nothing in an empty message, and the LLM is not reached unless `check-short-messages` is on. This is byte-for-byte how caption-less photos already behave (`transform` sets `message.Image` for a photo with no caption too), not a new exposure — see the media-only note above for the locator/`--max-short-msg-count` side effect. Gating the guard on `settings.Meta.DocumentsOnly` was rejected — it would leak a detector concern into the listener
