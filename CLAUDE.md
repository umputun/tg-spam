...
- The intake guard at `listener.go:390-391` gained `!msg.WithDocument` unconditionally — not gated on the setting, same treatment as `WithVideo`/`WithVideoNote`. With the check *disabled*, caption-less documents now reach the detector and locator where they previously did not: `Detector.Check` treats the empty text as short and returns early without reaching the LLM unless `check-short-messages` is on, but the message still counts toward `--max-short-msg-count` and duplicate detection, matching how caption-less photos/videos already behave (see the media-only note above). Gating the guard on `settings.Meta.DocumentsOnly` was rejected — it would leak a detector concern into the listener
- No dedicated `--meta.document-text-len` knob. Threshold is `settings.MinMsgLen`, like `videos` and `audio`; images have their own knob only because a separate need for it emerged in practice
