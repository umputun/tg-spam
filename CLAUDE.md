...
- The user-report dispatch also requires `update.Message.Chat.ID == l.chatID`. `DirectUserReport` always acts on the monitored group (deletes the command by its ID there, stores the report with `primChatID`), so a report from any other chat, `TestingIDs` chats and the admin chat included, would delete an unrelated message with the same ID and later ban or delete by foreign IDs. Such a reply falls through to `procEvents`, which drops it for a non-allowed chat and spam-checks it as an ordinary message in a testing chat. Same reason `isAnonymousGroupAdmin` compares against `l.chatID`, never `isChatAllowed`
