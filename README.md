...
- `--jev.threshold` (default `0.30`) is the probability at or above which a message is spam. Leave it alone unless you see a reason to move it. On 148 labeled messages from a real chat it caught 80 of 86 spam and flagged none of the 62 legitimate ones, and every value from `0.25` to `0.40` flagged none of them, so the default is not sitting on an edge: the highest-scoring legitimate message scored `0.24`. If you see false positives, raise it - `0.35` and `0.40` cost a few catches and stayed clean. If obvious spam gets through, `0.25` was still clean on that data. Expect somewhat worse than those numbers in your own chat: the threshold was picked after seeing those scores, and the measurement joined a post to its quoted text with a newline where the shipped code fuses them.
